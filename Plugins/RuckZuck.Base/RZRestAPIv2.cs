                ...
                using (HttpClient hClient = new HttpClient())
                {
                    try
                    {
                        ...
                        try
                        {
                            ...
                            {
                                _sURL = tReq.Substring(0, queryIndex);
                                var query = tReq.Substring(queryIndex + 1);
                                foreach (var part in query.Split('&'))
                                {
                                    var kv = part.Split(new[] { '=' }, 2);
                                    if (kv.Length == 2 && kv[0] == "customerid")
                                        CustomerID = Uri.UnescapeDataString(kv[1]);
                                }
                            }
